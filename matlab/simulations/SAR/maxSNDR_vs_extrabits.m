...
SNDRs = zeros(length(Bits_tested),length(extra_caps_tested));
...
%     % ideal thresholds
    ...
    ideal_SNDR = calculate_SNDR(ideal_digi_out,analog_in,N);
...
for mc = 1:MC_runs
    ...
    for N_bits_caps = extra_caps_tested
        ...
        caps = derrive_caps(N_bits + N_bits_caps, ...
            mismatch_LSC0 * sqrt(L_caps), LSC0/(L_caps));

        pos_thresholds = cap_to_thr(caps,Vlow,Vhigh);

        [~, idx] = min(abs(pos_thresholds(:) - ideal_thresholds(:).'), [], 1);
        best_thresholds = pos_thresholds(idx);

        % interpolate the thresholdsj
        % interpolthr = zeros(length(best_thresholds),1);
        % interpolthr(1:2:end) = best_thresholds(1:2:end);
        % for i=2:2:length(best_thresholds)-1
        %     interpolthr(i) = (best_thresholds(i-1) + best_thresholds(i+1))/2;
        % end
        % interpolthr = interp1(1:2:length(interpolthr)*2,interpolthr,2:2:length(interpolthr)*2-1);
