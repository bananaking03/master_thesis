...
init_thresholds(30) = init_thresholds(30) + Vinc*0.9;
...
%         converge_times(i) = 0;
%     else
%         SNDR_intercept = (SNDRs_cases(i) - min(SNDRs))*0.9 + min(SNDRs);
%         j = 1;
%         done = false;
%         while (done == false)
...
% end
    if max(SNDRs) <= SNDRs(1)
        ...
        SNDR_intercept = (SNDRs_cases(i) - min(SNDRs))*0.95 + min(SNDRs);
        j = 1;
        done = false;
        while (done == false)
            if (j == cal_cycles)
...
converge_times_math = 1.5*cal_len*L./(const_vals);
% converge_times_math_fit = 10*N_vals*L/(cal_constant);
% converge_times_math_fit = 100*N_vals*L;
converge_times_math_fit = cal_len*log(0.05)./(2*log(1-const_vals./L));

%% SNDR plot
figure;
yyaxis left
semilogx(cal_consts, SNDRs_cases, '*','MarkerSize',12);
hold on
semilogx(cal_consts, SNDRs_cases_max, '*','MarkerSize',12,'Color',[0.7 0 1 0.3]);
semilogx(const_vals, SNDR, 'LineWidth', 2,'Color',[0 0 1 0.3]);
...
title('SNDR and convergence time for each cal constant');
grid on;

yyaxis right
semilogx(cal_consts, converge_times, '.','MarkerSize',12);
semilogx(const_vals, converge_times_math, 'Linewidth',2,'Color',[255/255 165/255 0 0.3]);
ylabel('Convergence time (amount of cycles)')
set(gca, 'YScale', 'log')   % <-- THIS makes right axis logarithmic
set(gcf, 'Color', 'white')
set(gca, 'Color', 'white')
set(gca, 'XColor', 'black')
set(gca, 'GridColor', 'black')
legend('simulation averaged SNDR', 'simulation max SNDR','simulation converge times', 'theoratical converge time')
hold off;
