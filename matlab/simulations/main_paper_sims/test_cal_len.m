...
for i = 1:num_cases
    ...
    [digi_out, thr, H_delta_diffs] = flash_adc_dither_sim(analog_in, cal_len, cal_cycles, ...
        cal_constant, 0,init_thresholds, Vhigh, Vlow, Vinc, N_bits, non_lin_parameters);
    ...
    % H_delta_diff_history(:, (i-1)*(cal_cycles+1)+1:i*(cal_cycles+1)-1) = H_delta_diffs;
end

figure;
semilogx(cal_lens, SNDRs, '*');
hold on;
...
title('SNDR for each cal length');
grid on;
hold off;
...
for i = 1:num_cases
    ...
    tau(i) = -cal_lens(i)/p(1);   % convergence time constant
end
xlabel('Calibration Iteration');
...
figure;
semilogx(cal_lens,tau, '*');
hold on;
xlabel('Calibration length');
ylabel('cycles to convergence');
title('Convergence speed (cal_len = 10^-6');
hold off;
...
%% --- Animation Setup ---
case_to_show = 5; % choose which calibration length to animate (index in cal_lens)
